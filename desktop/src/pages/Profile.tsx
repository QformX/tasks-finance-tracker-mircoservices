    ...
    <div className="flex-1 overflow-y-auto w-full bg-background-dark">
      <div className="max-w-4xl mx-auto p-6 md:p-10 lg:p-12 pb-24">
        <header className="mb-10">
          ...
          <p className="text-text-secondary">Manage your personal information and security preferences.</p>
        </header>

        <div className="space-y-8">
          {/* Personal Information */}
          <section className="bg-surface-dark border border-white/5 rounded-3xl p-6 md:p-8">
            <h2 className="text-xl font-bold text-white mb-8">Personal Information</h2>
            <div className="flex flex-col md:flex-row gap-8">
              <div className="shrink-0 flex flex-col items-center md:items-start gap-4">
                ...
                <span className="text-xs font-semibold text-text-secondary">Allowed *.jpeg, *.jpg, *.png</span>
              </div>
              <div className="flex-1 space-y-5 w-full">
                ...
                <div className="pt-4 flex justify-end">
                  <button className="px-6 py-3 bg-white text-black font-bold rounded-xl hover:bg-gray-200 transition-colors shadow-lg shadow-white/5 text-sm">Save Changes</button>
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="bg-surface-dark border border-white/5 rounded-3xl overflow-hidden">
            <div className="p-6 md:p-8 border-b border-white/5 flex flex-wrap gap-4 justify-between items-center bg-white/[0.02]">
              <div>
                ...
                <p className="text-text-secondary text-sm mt-1">Manage devices where you're currently logged in.</p>
              </div>
              <button 
            ...
            <div className="divide-y divide-white/5">
              {loadingSessions ? (
                <div className="p-6 text-center text-text-secondary text-sm">Loading sessions...</div>
              ) : sessions.length === 0 ? (
                <div className="p-6 text-center text-text-secondary text-sm">No active sessions found.</div>
              ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-bold text-white text-sm">
                          {session.user_agent ? (session.user_agent.length > 30 ? session.user_agent.substring(0, 30) + "..." : session.user_agent) : "Unknown Device"}
                        </span>
                        {session.is_current && (
                          <span className="px-2 py-0.5 rounded text-[10px] font-extrabold bg-green-500/10 text-green-400 border border-green-500/20 tracking-wide">CURRENT</span>
                        )}
                      </div>
                      <p className="text-xs text-text-secondary truncate">
                        {session.ip_address || "Unknown IP"} • Active {new Date(session.created_at).toLocaleDateString()}
                      </p>
                    </div>
                    {!session.is_current && (
                      <button 
          ...
          {/* Preferences */}
          <section className="bg-surface-dark border border-white/5 rounded-3xl p-6 md:p-8">
            <h2 className="text-xl font-bold text-white mb-8">Preferences</h2>
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div className="pr-8">
                  <p className="text-sm font-bold text-white">Email Notifications</p>
                  <p className="text-xs text-text-secondary mt-1">Receive weekly summaries and important alerts.</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="pr-8">
                  <p className="text-sm font-bold text-white">Desktop Push</p>
                  <p className="text-xs text-text-secondary mt-1">Get notified immediately on your desktop when a task is due.</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="pr-8">
                  <p className="text-sm font-bold text-white">Theme Sync</p>
                  <p className="text-xs text-text-secondary mt-1">Match application theme with system settings.</p>
                </div>
                <button className="relative inline-flex h-6 w-11 items-center rounded-full bg-primary transition-colors focus:outline-none">
                  <span className="translate-x-6 inline-block h-4 w-4 transform rounded-full bg-white transition-transform"></span>
                </button>
            ...
            <div className="relative z-10">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <p className="text-text-secondary text-sm max-w-xl leading-relaxed">
                  Permanently remove your Personal Account and all of its contents from the TaskMaster platform. This action is not reversible, so please continue with caution.
                </p>
