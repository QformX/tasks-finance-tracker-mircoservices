    ...
    <div className="flex-1 overflow-y-auto w-full bg-background">
      <div className="max-w-4xl mx-auto p-6 md:p-10 lg:p-12 pb-24">
        <header className="mb-10">
          <h1 className="text-2xl lg:text-3xl font-bold text-text-950 mb-2">{t("settings_title")}</h1>
          <p className="text-text-secondary">{t("settings_desc")}</p>
        </header>

        <div className="space-y-8">
          {/* General Settings */}
          <section className="bg-surface border border-text-950/5 rounded-3xl p-6 md:p-8">
            <h2 className="text-xl font-bold text-text-950 mb-8">{t("general")}</h2>
            
            <div className="space-y-6">
              <div className="flex flex-col gap-2">
                <label className="text-sm font-bold text-text-950">{t("language")}</label>
          ...
          {/* Profile Link */}
          <section className="bg-surface border border-text-950/5 rounded-3xl p-6 md:p-8 flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold text-text-950 mb-1">{t("profile_settings")}</h2>
              <p className="text-text-secondary text-sm">{t("profile_settings_desc")}</p>
            </div>
            <Link 
              to="/profile" 
              className="flex items-center gap-2 px-5 py-2.5 bg-text-950 text-surface font-bold rounded-xl hover:bg-text-950/80 transition-colors shadow-lg shadow-text-950/5 text-sm"
            >
