...
### 5️⃣ Фоновые задачи (Core Worker)

**Core Worker** - отдельный процесс, который слушает очередь событий `core_events` в RabbitMQ.

#### Функционал:
- **Обработка удаления пользователя (`UserDeleted`):**
  - При получении события `UserDeleted` от Users Service:
  - Удаляет все задачи пользователя
  - Удаляет все покупки пользователя
  - Удаляет все категории пользователя
  - Удаляет все умные фильтры пользователя
  - Обеспечивает соответствие GDPR (право на забвение)
